...
##### Answer 1.1: Definition of Software Architecture
...
##### Answer 1.2
*Same underlying definition as Answer 1.1 above — see the Shaw & Garlan / IEEE 1471 definitions and the diagram there. At 3 marks, state the Shaw & Garlan definition (components + connectors) and one sentence on why it matters (it is the first artifact where quality attributes like performance and maintainability get engineered in).*
...
##### Answer 1.3: Why Software Architecture is Important
...
*This question also asks "what is partitioning the architecture, with an example" — that half of the answer is identical to Answer 1.4 below (Partitioning the Architecture); at 6 marks, cover the three importance reasons above plus a brief summary of horizontal OR vertical partitioning with one example, rather than both partitioning types in full.*
...
##### Answer 1.4: Partitioning the Architecture — Horizontal vs. Vertical Partitioning
...
###### Why Partitioned Design is Required
Partitioning is required because an un-partitioned, monolithic module structure has poor testability (a fault anywhere affects the whole system), poor maintainability (a change to one concern risks breaking unrelated concerns), and blocks concurrent team development (multiple engineers cannot safely work on the same undivided module). Dividing the architecture along either the horizontal (layer) or vertical (control) dimension isolates these concerns so that they can be tested, maintained, and developed independently.
...
##### Answer 1.5: How the Analysis Model Translates into the Architectural Design Model
...
##### Answer 2.1: Influence of Data Warehousing and Data Mining on System Architecture
...
##### Answer 2.2: Step-by-Step Data Design Process (E-Commerce Worked Example)
...
*Questions 3.1–3.7 all draw on the same reference library of five architectural styles (Data-Centered, Data-Flow, Call-and-Return, Layered, Object-Oriented), defined once at the end of this group as the "Style Reference Library" so the full detail isn't repeated seven times. Each question below is answered by naming which specific style(s) it calls for and pointing to that entry — jump to the Style Reference Library for the full structure/diagram/pros/cons of each one.*
...
##### Answer 3.1
Explain **Style D: Layered Architecture** and **Style B: Data-Flow Architecture** in full, from the Style Reference Library below (structure, diagram, and advantages/limitations for each — no other styles are needed for this question).
...
##### Answer 3.2
Explain **Style C: Call-and-Return Architecture** and **Style D: Layered Architecture** in full from the Style Reference Library below. At 10 marks, also include the "How Layered Relates To, But Is Not Identical To, Call-and-Return" comparison at the end of this group, since the question pairs exactly these two styles.
...
##### Answer 3.3
The question permits choosing any two. **Style A (Data-Centered)** and **Style C (Call-and-Return)** are recommended as a pair, since together they cover both a data-driven style and a control-driven style, showing breadth. Explain both in full from the Style Reference Library below (any other pairing from the five is equally valid on an exam).
...
##### Answer 3.4: Definition of Architectural Style vs. Architectural Pattern
...
For "when Call-and-Return can be used, justified with an example," see **Style C: Call-and-Return Architecture** in the Style Reference Library below — its "Suitable Application" and diagram together justify when to use it (payroll processing, classic desktop applications, mathematical computation libraries — anywhere control naturally forms a top-down invocation hierarchy).
...
##### Answer 3.5
The suitable style is **Style A: Data-Centered Architecture (Repository & Blackboard)** — see the Style Reference Library below, which already uses the college blackboard system as its worked "Suitable Application" example with a matching diagram.
...
##### Answer 3.6
See **Style A: Data-Centered Architecture (Repository & Blackboard)** in the Style Reference Library below. At 4 marks, cover the structure/components bullet, the Repository-vs-Blackboard distinction, and the college blackboard system example — the diagram and advantages/limitations are optional extras at this mark weight.
...
##### Answer 3.7
This is the only question in the group that asks for full breadth. Explain all five styles (A through E) from the Style Reference Library below.

---

### Style Reference Library (Shared by All of Group 3)

#### Detailed Breakdown of the Five Prescribed Architectural Styles
...
* **Suitable Application:** Unix shell command pipelines, Compilers (Lexical Analyzer → Parser → Semantic Analyzer → Code Generator), Image processing pipelines, Signal processing.
...
* **Suitable Application:** OSI 7-Layer Networking Model, Enterprise E-Commerce Web Applications (Presentation Layer → Business Logic Layer → Data Access Layer → Database Layer).
...
#### How Layered Architecture Relates To, But Is Not Identical To, Call-and-Return Architecture
...
##### Answer 4.1
This is the same underlying question as the partitioning half of Question 1.3 and all of Question 1.4 — it does not belong to this group's ACD/component-refinement content. See **Answer 1.4: Partitioning the Architecture — Horizontal vs. Vertical Partitioning** in Topic Group 1 for the full answer with diagram. At 3 marks, state one partitioning type (horizontal or vertical) with its one-line example rather than both.
...
##### Answer 4.2: Architectural Context Diagram (ACD) Structure and Categories
...
*Questions 5.1–5.3 are three phrasings of essentially the same question (the transform-mapping procedure) at three different mark weights, so they share one full answer below rather than being split.*
...
##### Answer 5.1: Full Transform Mapping Answer (10 Marks)
At 10 marks, give the complete answer below: the flow-characteristics introduction, all seven mapping steps, and at least one fully worked example (SafeHome or the ATM system) with both the DFD-boundary analysis and the resulting component hierarchy.
...
##### Answer 5.2 (7 Marks)
Same content as Answer 5.1 below. At 7 marks, cover the seven mapping steps in full but include only one worked example (either SafeHome or the ATM system, not both).
...
##### Answer 5.3 (6 Marks)
Same content as Answer 5.1 below. At 6 marks, cover the flow-characteristics introduction and the seven mapping steps; a worked example is optional at this weight — a brief one-paragraph sketch of the incoming/transform/outgoing boundary idea (without the full DFD diagrams) is sufficient.

---

### Full Worked Answer (Shared by Questions 5.1–5.3)
...
* Optimize **Fan-in** (high fan-in for reusable utility modules) and **Fan-out** (keep control fan-out ≤ 7).
...
* **Questions 1.1, 1.2, 1.3, 1.4, 1.5** → Answered in **Topic Group 1**
* **Questions 2.1, 2.2** → Answered in **Topic Group 2**
* **Questions 3.1, 3.2, 3.3, 3.4, 3.5, 3.6, 3.7** → Answered in **Topic Group 3**
* **Questions 4.1, 4.2** → Answered in **Topic Group 4** (Question 4.1's actual content is a pointer to Answer 1.4 in Topic Group 1, since it asks about partitioning, not ACD/component refinement)
* **Questions 5.1, 5.2, 5.3** → Answered in **Topic Group 5**
