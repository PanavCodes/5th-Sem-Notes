...
2. **Zero-Sum Games:**  
   A game where the total payoff to all players is constant across all outcomes. In a two-player zero-sum game, any gain by MAX represents an equal loss for MIN:
...
Level 0 (Root)       [ MAX ]             <- Initial State (s0)
                    /       \
Level 1          [ MIN ]   [ MIN ]       <- Ply 1 (MAX's move)
                 /     \   /     \
Level 2       [MAX]  [MAX][MAX]  [MAX]   <- Ply 2 (MIN's move)
               |      |    |      |
Leaves        (3)    (12) (8)    (2)     <- Terminal States (Utility Values)
```
...
The **Minimax Algorithm** is a recursive depth-first search algorithm used to calculate the optimal move for MAX in a two-player, zero-sum, perfect-information game tree.

---

### 3.2 Step-by-Step Procedure

1. **Generate Full Game Tree:** Perform a Depth-First Search (DFS) traversal down to the terminal leaf states.
...
* **Space Complexity: $O(b \cdot m)$**
  * Minimax performs a Depth-First Search (DFS), keeping only the current path from the root to the leaf and sibling nodes in memory.
...
1. **Alpha Cutoff (at MIN Node):**  
   If a MIN node $n$ finds a child value $v \le \alpha$, the search below $n$ is terminated immediately.
...
> ⭐ **Must Remember**  
> Alpha-Beta Pruning **returns the EXACT same move as Minimax**. It does NOT make approximations or compromise decision quality.
...
6. **At Node I (MIN):**
   ...
   * **Check Pruning Condition at Node I:**
     $$\alpha = 8, \quad \beta = -47 \implies \alpha \ge \beta \quad (8 \ge -47) \quad \text{\textbf{TRUE!}}$$
...
20. **At Node M (MIN):**
    ...
    * **Check Pruning Condition at Node M:**
      $$\alpha = -19, \quad \beta = -49 \implies \alpha \ge \beta \quad (-19 \ge -49) \quad \text{\textbf{TRUE!}}$$
...
25. **Back to Node G (MAX):**
    ...
    * **Check Pruning Condition at Node G:**
      $$\alpha = 43, \quad \beta = -19 \implies \alpha \ge \beta \quad (43 \ge -19) \quad \text{\textbf{TRUE!}}$$
...
| **G** | MAX | 43 | $\alpha = 43, \beta = -19$ | **Pruned child subtree O ($\alpha \ge \beta$).** |
...
3. **Subtree C (MIN):**
   ...
   * **Check Pruning Condition at Node C:**
     $$\alpha = 3, \quad \beta = 2 \implies \alpha \ge \beta \quad (3 \ge 2) \quad \text{\textbf{TRUE!}}$$
   * ✂️ **ALPHA CUTOFF AT NODE C!**
   * **Remaining children of C (leaves $x, y$) are PRUNED.**
   * $\text{Value}(C) \le 2$.
4. **Subtree D (MIN):**
   * Inherits $(\alpha = 3, \beta = +\infty)$.
...
* **Pruned Nodes:** All sibling leaves under $C$ after leaf $2$.
...
| **Decision Quality** | Exact optimal decision. | **Identical exact optimal decision.** |
...
4. ❌ **Mistake:** Thinking Alpha-Beta gives an approximate or different answer than Minimax.  
   * **Correction:** Alpha-Beta returns the EXACT SAME root value and move decision as Minimax.
